...
struct IVarianMemCtrl: IOSWriter
{
    virtual void cloneFrom(const Body & from, Body * pOut) = 0;
    virtual void destroy(const Body & other) = 0;
...
public:
    ~VarianMemCtrl(){ delete m_pWriter; }
    ...
    {
        Body res;
        res.buffer = SharedPtr<uint8_t>(new uint8_t[sizeof(T)]);
        res.id = ID_Ver<T>::ID;
        res.size = sizeof(T);
    ...
    virtual void cloneFrom(const Body & from, Body * pOut)
    {
        ...
        T * ptr = (T*)from.buffer;
        pOut->buffer = SharedPtr<uint8_t>(new uint8_t[sizeof(T)]);
        pOut->size = sizeof(T);
        pOut->id = ID_Ver<T>::ID;
        pOut->ctrl = from.ctrl;
    ...
    virtual void destroy(const Body & other)
    {
        if ( !isBodyOK(other) ){
            throw std::bad_cast("Varian: bad cast");
        }
        T * ptr = (T*)other.buffer;
        ptr->~T();
    }
